...
[![Publication](https://img.shields.io/badge/Publication-Half%20Duplex%20Reliable%20File%20Transfer%20over%20UDP%20Protocol-red?style=flat&logo=googledocs&logoColor=red)](./Papers/Half-Duplex%20Reliable%20File%20Transfer%20over%20UDP%20Protocol.pdf)

Implement naive flow control mechanism using **(a)** Stop and Wait Protocol and **(b)** Selective and Repeat Protocol. Transfer files (Text, Image, Audio, Video) using UDP protocol. If the connection is suddenly terminated during the process, you have to start over; it simply resumes the process, not restarts from the beginning.

Write a socket program in C for Multimodal File Transmission using UDP with the Half-Duplex Stop-and-Wait protocol. The program/protocol should support the following properties/mechanisms

1. The protocol will send any type of file
...
Implement a reliable file transfer system over the UDP protocol. Since UDP itself is unreliable (packets may be lost, duplicated, or arrive out of order), you must design a mechanism to ensure reliability using techniques such as segmentation, sequencing, acknowledgements, and retransmission.
...
- ### Acknowledgement Handling
  ...
  The receiver must send back an ACK containing:
  - Sequence Number of the successfully received segment
  - File identifier (to handle multiple file transfers, if extended later)

- ### Reassembly of File

  The receiver collects all the received segments in order. Write them back into the original file format after successful transfer.
...
- Run the `server` file using the command below:
...
- The client is requested to provide the files.
...
>
> - Now go to the [Wireshark][def1] window and filter the packets by typing the command below in the filter bar.
>
>   ```ini
>   udp.port == 8080
>   ```
